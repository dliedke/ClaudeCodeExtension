...
- **Dead-spawn retry + launch log** (issue #73): the conhost spawn in `StartEmbeddedTerminalAsync` re-spawns once when the launched conhost exits before its window is found (user-visible as a console window flashing ~0.1s then closing, panel left blank); a second failure shows a MessageBox instead of failing silently. NO retry when the launcher is alive but windowless (would pile up live terminals). Launch steps (teardown, spawn PID, whether VS had a console attached, find/SetParent failures with Win32 errors, exit codes) append to `%LocalAppData%\ClaudeCodeExtension\terminal-launch.log` via `LogTerminalLaunch()` (best-effort, never throws, resets at 512 KB) — `Debug.WriteLine` is compiled out of Release builds, so this log is the only way to diagnose user machines; ask reporters to attach it
- **Embed failure honesty + panel self-repair** (issue #73): both embed paths verify `SetParent` actually took (`SetParent` returning zero is ambiguous — a top-level window's previous parent is also zero, so success is confirmed via `GetParent`); after 3 failed attempts they report via MessageBox + launch log instead of falling through claiming success (which left a hidden, parentless window = blank panel; the orphan is closed by the next restart via `terminalHandle`). `EnsureTerminalPanelReady()` recreates `terminalPanel` and re-hosts it in `TerminalHost` when it is missing/disposed — previously a dead panel made the launch silently bail, and since the dead panel persisted, *every* later restart stayed blank until the project was reopened (the only failure mode matching that exact symptom)
