...
- **Console-attach leak guard** (critical): the watcher `AttachConsole`s **VS itself** each tick. If VS is left attached, the **next `conhost.exe` spawn fails to create its window and the terminal renders blank** (most visible after switching solutions). Two guards: `ResetAgentCompletionForSolutionChange()` stops the watcher + dismisses the info bar on `OnBeforeCloseSolution` and the workspace-changed branch of `OnWorkspaceDirectoryChangedAsync`; and `EnsureNoConsoleAttached()` calls `FreeConsole` under a bounded `_consoleSnapshotLock` (`Monitor.TryEnter` w/ timeout), run after `StopExistingTerminalAsync()` and at the start of `ExecuteAgentFinishActionAsync` (no-op when VS has no console)
